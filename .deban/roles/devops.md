...
| 2026-06-07 | Absolute `/`-prefixed asset paths + SW `scope:'/'` | Would 404 on GitHub Pages project sites (served under `/<repo>/`), and the broad SW scope would fail registration. Switched everything to relative paths + default SW scope. |

## Lessons
- Use relative asset paths from day one: they work at the site root AND under a sub-path (GitHub Pages), locally and deployed; absolute `/` paths silently assume a root deploy. — from dead end on 2026-06-07
...
## Session Log
2026-06-07 — Deployed to GitHub Pages: https://kai-denrei.github.io/geejisuraidaa/ (public repo kai-denrei/geejisuraidaa, main branch, .nojekyll). Converted all paths to relative for sub-path serving. Verified deployed: all assets 200, manifest valid (standalone, maskable), SW+modules served as JS, iOS tags + HTTPS present. Token bca8d62d.
